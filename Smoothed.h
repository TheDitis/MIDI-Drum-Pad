...
class Smoothed {
  private:
    int smoothMode;
    int smoothReadingsFactor = 10; // The smoothing factor. In avergare mode, this is the number of readings to average.
    int smoothReadingsPosition = 0; // Current position in the array
    int smoothReadingsNum = 0; // Number of readings currently being averaged
    T *smoothReading; // Array of readings
  public:
    Smoothed();
    ~Smoothed(); // Destructor to clean up when class instance killed
    bool begin (int smoothMode, int smoothFactor = 10);
...
template <typename T>
bool Smoothed<T>::begin (int mode, int smoothFactor) {
  smoothMode = mode;
